...
一个进程由唯一的进程标识符process ID表示，这个上限是32767，由一个pidmap-array位图管理，pid的上限可以通过写入/proc/sys/kernel/pid_max这个文件来修改，在64位中，pid的- [内核模块学习](#内核模块学习)
- [1](#1)
  - [2](#2)
    - [内核模块学习](#内核模块学习)
      - [一、进程](#一进程)
        - [1) 进程描述符](#1-进程描述符)
        - [2) 进程状态](#2-进程状态)
        - [3) pid 和 tgid](#3-pid-和-tgid)
        - [4) 进程描述符处理](#4-进程描述符处理)
      - [二、内核数据结构](#二内核数据结构)
        - [1) list 双向链表](#1-list-双向链表)
          - [定义](#定义)
          - [操作函数及宏定义](#操作函数及宏定义)
        - [2) spinlock\_t 自旋锁](#2-spinlock_t-自旋锁)
          - [定义](#定义-1)
          - [操作方法](#操作方法)
        - [3) mutex 互斥锁](#3-mutex-互斥锁)
          - [定义](#定义-2)
          - [操作方法](#操作方法-1)
        - [4. acomic\_t 原子变量](#4-acomic_t-原子变量)
          - [定义](#定义-3)
          - [操作函数](#操作函数)
          - [原子位操作函数](#原子位操作函数)
