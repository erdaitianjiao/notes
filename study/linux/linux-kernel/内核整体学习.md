...
在32位系统上虚拟内存有4g，用户态进程可以访问前3个g，内核态可以访问后面1gb空间

### 内核的线性空间布局
虚拟内存也是有限制的，内核的1gb空间不能直接映射1g的物理内存，还有留有空间有专门的作用，也有一些空间要留作动态映射(ioremap)等

空间有限的情况下，内核启动的时候会将一部分内存直接映射到线性空间，剩下的部分用于满足其他要求，必须另做映射才能访访问，直接映射的部分是low memory 剩下的部分是high memory

直接映射指的是映射得到的虚拟地址 va 和物理地址 pa 有直接的关系 va = pa + PAGE_OFFSET

x86上物理地址空间是可以大于4gb的，但是线性空间只能是4gb，但是x86_64就不用分low和high

| 名 称 | 描 述 |
| --- | --- |
| high_memory | Low Memory 和 High Memory 的分界，线性地址，<br>• x86 上：high_memory = max_low_pfn<<PAGE_SIZE + PAGE_OFFSET<br>• x86_64 上：high_memory = max_pfn<<PAGE_SIZE + PAGE_OFFSET |
| PAGE_SIZE | 表示一页内存使用的二进制位数，一般等于 12 |
| PAGE_OFFSET | 内核线性空间的起始 |
| FIXADDR_TOP | 固定映射的结尾位置 |
| FIXADDR_START | 固定映射的开始位置 |
| VMALLOC_END | 动态映射区的结尾 |
| VMALLOC_START | 动态映射区的开始 |

![x86内核线性地址映射](./img/x86内核线性空间布局.png)

x86的内核地址分为
| 地址范围 / 关键地址 | 区域名称 | 描述 |
| :--- | :--- | :--- |
| 0xFFFFFFFF  | 最高 4KB | 不可用区域 |
| 0xFFFFF000  | 固定映射区 (FIXADDR) | APIC、FDT 等固定用途，编译时确定 |
| 0xFFFFB000  | CPU Entry Area | 存放 `cpu_entry_area`（每 CPU 的 GDT、TSS 等） |
| 0xFFC00000  | 永久映射区 (PKMAP) | 临时映射 High Memory 页，最多 512/1024 个槽位 |
| 0xFF800000  | *(Gap)* | 2 pages gap (间隙) |
| 0xFF7FFFFF  | 动态映射区 | vmalloc / ioremap / vmap (~120MB，运行时可伸缩) |
| 0xF8800000 <- `VMALLOC_START` | | |
| *(8MB Hole)* | *(Hole)* | 8MB hole（越界检测，故意留空） |
| high_memory | 直接映射区 | Low Memory ≤ 896MB，va = pa + 0xC0000000 |
| 0xC0000000  <- `PAGE_OFFSET` | | |

线性地址用户空间和内核空间比例是3:1，3gb的用户空间每个进程独立拥有且互不影响，也就是进程自己负责维护这部分页表，称为用户页表。也只需要维护自己使用的那部分页表，并不需要维护整套的用户页表。内核空间的1gb所有进程共有

![x64的内核空间分配](./img/x64的内核线性空间分配.png)

x64的情况下，支持五级页表的情况下，最大支持52位物理地址，否则支持46位物理地址，这两种情况的线性空间的直接映射区都足以包含所有的物理地址

#### 直接映射区

直接映射区就是将物理地址直接映射到线性地址，虚拟地址va等于物理地址pa + PAGE_OFFSET
如果内存没有不够MAX_MEM的情况下或者预留high mem(x64没有)的情况下，全部的物理内存会被映射到直接映射区，如果系统内存是大于896mb的时候，内存就不能全部映射过去

直接映射区一旦完整，系统运行期间不会改变

#### 动态映射区

动态映射区是满足其他几个区不能满足的要求，他是内核线性空间中最灵活的区域，常见的ioremap、mmap一般都需要使用它


内核提供vm_struct来获取线性空间及其映射的情况
| 字 段 | 类 型 | 描 述 |
| :--- | :--- | :--- |
| addr | void * | 线性空间的起始地址 |
...
申请内存流程
``` c
get_vm_area(VMALLOC_START, VMALLOC_END) {
    get_vm_area_node() {
        // 红黑树里面查找空闲区间 (O(logn))
        // 找到 创建vmap_are记录 插入红黑树 和 链表
        // 填充 vm_struct 返回给调用者
        // 调用者拿到后 map_vm_area() 把物理内存映射上去
    }
}
```
使用红黑树加链表的原因
红黑树用于 快速查找哪块区间空闲并足够大 
链表用于 vmap_start 按照 vm_start 从小到大穿起来

释放流程
```c
remove_vm_area();   // 解除页表映射
free_vm_area();     // 释放vm_struct 从红黑树和链表中删除vmap_area
```

#### 永久映射区
(x64没有)

用于给x86的机器访问超过 high 部分物理内存的一个入口

内核提供一个kmap函数将一页映射到这个区， kunmap用于取消映射，如果该部分不属于high mem就直接返回该page对应的的虚拟地址vaddr

x64不存在high mem所以调用kmap直接返回vaddr，所以逻辑上不存在永久映射区

kmap可能会引起系统睡眠，不可在中断环境里面使用

#### 固定映射区

这里面的区分为多个区域，每一个小区域都有特定的用途，如ACPI等

所有的区间里面有一个叫临时映射区域，每一个cpu都有自己的零临时映射区，内核提提供函数将物理页映射到这个区域，每个cpu都有自己的临时映射区，十几页，使用task_struct的kmap_ctrl的idx来记录，这个优点是不会睡眠，但是kmap会睡眠。另一方面，这个的使用是有显示的，kmap_atomic会禁止内核抢占，所以只能用于存放一些临时数据，数据处理完成后就释放
...
mmap将文件映射到内存，映射以后不再需要read/write来操作文件，可以像访问内存一样访问他们
```c
void * mmap (void * addr, size_t length, int prot, int flags, int fd, off_t offset);
int murunap (void * addr, size_t length);
```

内核将文件或者设备从offset开始，到offset+length结束的区间映射到addr开始的虚拟内存中，并将addr返回，映射后的内容的访问权限由port决定
...
| MAP_HUGETLB<br>MAP_HUGE_2MB<br>MAP_HUGE_1GB | 映射大页内存 |

私有共享是相对于其他进程来说的，MAP_SHARED在映射区的更新对于其他映射同一区域的进程可见，所作的也会体现在文件里面，可以使用msync控制协会文件，MAP_PRIVATE使用的是写时复制的策略，更新对同一区域的进程不可见，更新也不会写回文件

内核用vm_area_struct结构体来表示一个用户线性空间的区域
...
| vm_file | file * | 映射的文件对应的 file 对象 |
| vm_pgoff | unsigned long | 在文件内的偏移量 |
| vm_mm | mm_struct * | 所属的 mm_struct |

进程需要自己管理它所有的vm_area_struct对象，进程的mm_struct结构体有两个字段完成该任务，mmap字段是进程vm_area_struct对象组成的链表，mm_rb是所有vm_area_struct对象组成的红黑树的根，内核使用链表遍历对象，使用红黑树查找符合条件的对象

从应用的角度来看，mmap返回成功就意味着映射完成，但从实际上的驱动角度并不一定如此，很多驱动在mmap上并未实现，实际要在最后访问触发异常的时候才做最终的映射，这就用到了vm_ops字段
...
mmap() {  // 用户态
    ksys_mmap_pgoff() {
        do_mmap() {      // 核心
            ...
            calc_vm_prot_bits();    // prot/falgs 转 vm_flags
            mmap_region();          // 创建vma 插入 mm_struct

            // vma_set_anonymous    // 针对匿名页和的情况
        }
    }
...
1. 长度对其， len = PAGE_ALIGN(len)，你申请 0x50 实际给 0x1000

2. 找坑：get_unmapped_area()
addr == 0：内核在用户空间里搜一个空闲区间
addr != 0 && MAP_FIXED：直接抢地盘，即使已被占用也强制覆盖（不推荐）
MAP_FIXED_NOREPLACE：指定地址，被占用就返回错误（友好版）

3. 算权限 把 PROT_READ/PROT_WRITE 和 MAP_SHARED/MAP_PRIVATE 转成 vm_flags，同时检查文件权限（文件只读就不能 MAP_SHARED + PROT_WRITE）

VM_XXX vs VM_MAYXXX：
VM_READ      -> 这个 vma 当前可读
VM_MAYREAD   -> 这个 vma 可以被设置为可读（允许的范围）
一个是"当前权限"，一个是"允许的上限"。mprotect 可以在 MAY 范围内修改当前权限，但不能超出。
...
```c
vma = vm_area_alloc(mm);           // 分配 vma 对象
vma->vm_start = addr;              // 填起始地址
vma->vm_end = addr + len;          // 填结束地址
vma->vm_flags = vm_flags;          // 填权限
vma->vm_page_prot = vm_get_page_prot(vm_flags);  // 填页表属性
vma->vm_pgoff = pgoff;             // 填文件偏移

if (file)
    file->f_op->mmap(file, vma);   // 调驱动的 mmap
else
    vma_set_anonymous(vma);        // 匿名映射

vma_mas_store(vma, &mas);          // 插入 maple_tree
```
mmap 只负责找坑、建 vma，物理内存的映射由驱动决定，也可以完全不做

- 驱动实现mmap的方式
 
第一类：已有物理内存，直接映射（MMIO 最常见）

```c
// 驱动有自己的 MMIO 物理地址，直接映射给用户态
return vm_iomap_memory(vma, dev_data->io_addr, len);
```
...
remap_pfn_range(vma, vma->vm_start, page_to_pfn(page), size, ...);
```


第三类：延迟映射 mmap 时不映射，等缺页异常

```c
static int my_mmap3(struct file *file, struct vm_area_struct *vma) {
    vma->vm_ops = &my_file_mmap;  // 只设置 ops，不做映射
    return 0;
}
// 首次访问触发缺页异常 -> 内核回调 vm_ops->fault → 驱动再分配物理页
...
共享内存的本质是把同一段物理内存映射到各自的用户空间，虚拟地址可以不同，但是物理页是同一个


malloc不是直接调用mmap，而是盗用brk(改堆的上限)，但是本质上是一样的，申请一段连续的虚拟地址，物理页是访问的时候才分配，brk没有调用mmap来实现，但是实际上他是一个mmap

### /dev/mem 的用法

/dev/mem的用法就是把物理地址当成一个文件来读写，在文件的偏移量等于物理地址

![dev-mem流程](./img/dev-mem-用法.png)
1. 合法性检查，判断是否在合理范围内
2. 调用phys_mem_access_prot 看看是不是O_SYNC标志，如果是就禁用缓存
3. 调用remap_pfn_range映射指定大小的内存，得到在用户空间可以访问的虚拟内存

新版内核中只能访问mmio，不能访问dram了
